<template>
  <comment :comments="comments" @add="addNewReplay" @remove="removeReply" />
</template>
    ...
    refreshPage() {
      ...
      this.$get('/commentary/getInfoBySid', {
        ...
        for (let i = 0, len = allComments.length; i < len; i++) {
          ...
          // 2 再获取回复
          const nowCoId = allComments[i].id // 当前评论的id
          // 2.1 商家回复
          this.$get('/shopReply/getInfoByCoid', {
            coid: nowCoId
          }).then(res => {
            const allShopReply = res.data.data
            ...
            this.$get('/consumerReply/getInfoByCoid', {
              ...
              const allConsumerReply = res.data.data
